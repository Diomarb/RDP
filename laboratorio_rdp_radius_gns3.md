...
<!DOCTYPE html>
<html>
<head><title>Laboratorio GNS3</title></head>
<body style='background:#0b3d91;color:white;text-align:center;font-family:Arial'>
  <h1>Pagina personalizada IIS</h1>
  <p>Accedida via RemoteApp / RD Web Client</p>
</body>
</html>
"@

# Poner index.html como documento predeterminado (primero en la lista)
Import-Module WebAdministration
Add-WebConfiguration -Filter "/system.webServer/defaultDocument/files" -PSPath "IIS:\Sites\Default Web Site" -Value @{value="index.html"}
```
...
5. `ssh admin15@192.168.20.1` entra en modo privilegiado 15 automáticamente — OK.
6. `ssh user01@192.168.20.1` entra en modo usuario 1, requiere `enable` + `ClaseConfig2026` para subir de nivel — OK.
7. `show aaa servers` muestra el NPS como servidor activo con contadores de solicitudes — OK.
8. `debug radius` muestra el Access-Accept con `Cisco-AV-Pair = shell:priv-lvl=X` — OK.
